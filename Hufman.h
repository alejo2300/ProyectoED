#ifndef __HUFMAN__H__
#define __HUFMAN__H__

#include <stdio.h>
#include <stdlib.h>
//Constante altura maxima del arbol
#define MAX_TREE_HT 100

//Estructura de Base y Posicion
struct Retornables{
  char base;
  vector<int> arista;
};
...
vector<int> printArr(int arr[], int n)
{
    int i;
    vector<int> arisL;
    for (i = 0; i < n; ++i){
        printf("%d", arr[i]);
        arisL.push_back(arr[i]);
      }

    printf("\n");
  /*for(int i=0;i<arisL.size();i++){
      cout<<arisL[i];
    }*/
    return arisL;
}
...

{
    ...
    // and its code from arr[]
      ...
      if (isLeaf(root)) {
        ...
        nuevo.arista = printArr(arr, top);

        /*for(int i=0;i<nuevo.arista.size();i++){
          cout<<nuevo.arista[i];
        }*/
        cout<<"\n";
        cargarHuff(nuevo);
